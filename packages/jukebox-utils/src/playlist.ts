...
export class Playlist implements PlayableTrackList {
  static shuffle(tracks: PlayableTrack[]): PlayableTrack[] {
    const toShuffle = tracks.concat();
    const toReturn: PlayableTrack[] = [];
  ...
  shuffled: PlayableTrack[];
  custom: boolean;
  imageSrc?: string;
  album?: string;
  artist?: string;

  constructor(name: string, tracks: PlayableTrack[], custom: boolean) {
    this.name = name;
    this.tracks = tracks;
    this.custom = custom;
    ...
    this.album = this.tracks.length
      ? this.tracks[0].album || this.tracks[0].title
      : undefined;
